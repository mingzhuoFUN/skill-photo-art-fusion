...
一个面向 Codex 的中文照片艺术二次创作 Skill。它不是简单套滤镜，而是在保留照片身份、主体关系与空间逻辑的基础上，选择适合的主创区域，修复影响审美的问题，并加入一眼可见但有来源依据的艺术创作。
...
```text
使用 $photo-art-fusion-zh 对这张照片进行虚实结合的艺术二次创作。
```

也可以补充偏好，例如：

```text
使用 $photo-art-fusion-zh 进行二次创作。保留建筑主体，根据画面结构选择主创区域，艺术效果要明显，但避免全图覆盖同一种纹理。
```

Skill 会先判断照片身份、主体权重、结构与审美问题，再规划 `locked`、`primary`、`supporting` 和 `quiet` 区域，最后调用图像生成能力完成编辑并检查结果。

## 设计目标

本 Skill 重点解决以下常见问题：

1. 只选择天空等“绝对安全背景”创作，导致真正有潜力的区域被忽视。
2. 只有调色或修复，没有用户能直接感受到的 AI 创作点。
3. 同一种艺术纹理覆盖天空、地面、海洋和主体，造成密集、拼贴或模板感。
4. 人物权重被自动抬高，即使人物并不是照片主题也无法修改。
5. 没有明显主体时缺少创作依据。
6. 新增元素与原图没有来源关系，形成随意堆砌。

完整执行规则请查看 [`photo-art-fusion-zh/SKILL.md`](photo-art-fusion-zh/SKILL.md)。
